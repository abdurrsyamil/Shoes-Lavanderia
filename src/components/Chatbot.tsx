...
export const Chatbot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans" id="lavanderia-chatbot-root">
      ...
      {chatbotOpen && (
        <div 
          className="absolute bottom-18 right-0 w-[320px] md:w-[360px] h-[480px] bg-[#121212] border-2 border-black rounded-2xl shadow-2xl flex flex-col overflow-hidden transition-all duration-300 transform translate-y-0 opacity-100 scale-100"
          id="lavanderia-chat-window"
        >
          {/* Window Header */}
          <div className="bg-black p-4 border-b border-neutral-900 flex items-center justify-between">
            ...
            <button 
              ...
              className="text-neutral-500 hover:text-white transition-colors cursor-pointer p-1 bg-transparent border-0"
          ...
          <div 
            className="px-4 py-2 bg-neutral-950 border-t border-neutral-900 overflow-x-auto whitespace-nowrap flex gap-2" 
          ...
          >
            {SUGGESTED_CHIPS.map((chip, idx) => (
              <button 
                ...
                className="bg-neutral-900 hover:bg-[#ffb700] hover:text-black text-[#ffb700] text-[10px] font-mono tracking-wider font-bold py-1.5 px-3 border border-neutral-850 rounded-full transition-all cursor-pointer whitespace-nowrap flex-shrink-0"
              >
                {chip.label}
              </button>
            ))}
          ...
          <div className="p-3 bg-neutral-950 border-t border-neutral-900 flex items-center gap-2">
            <input 
              ...
              className="flex-1 bg-[#121212] border border-neutral-800 text-white placeholder-neutral-500 rounded-xl py-2 px-3.5 text-xs font-mono tracking-wide focus:outline-none focus:border-[#ffb700] transition-colors"
              id="lavanderia-chat-input"
            />
            <button 
              onClick={handleSend}
              className="bg-[#ffb700] hover:bg-[#e0a100] text-black p-2 rounded-xl transition-all border border-black cursor-pointer bg-transparent"
